...
│   ├── coverage.md                         # Coverage manifest (auto-generated by report-pencil-coverage)
...
The project has 10 custom Claude Code skills for frontend development (React, Astro, and HTML):
...
4. **create-pencil-design**: Generates Pencil (.pen) design frames for all Page wireframes in `docs/wireframes/`. Reads `spec.md` to resolve Page vs Component types — builds ALL Component wireframes as standalone reusable Pencil components first (components-first workflow), then generates Page designs that reference them via component instances. Reads `pencil/coverage.md` to skip already-applied wireframes (incremental design). Updates the coverage manifest after completion.
5. **report-pencil-coverage**: Inspects the current `.pen` file, cross-references with `spec.md` and `docs/wireframes/`, and writes a coverage manifest (`pencil/coverage.md`) recording which wireframes have been applied. Read by `create-pencil-design` for incremental design — prevents duplicate work when new wireframes are added.
6. **generate-pencil-images**: Generates or regenerates AI images (WebP) in `pencil/images/` for nodes in the selected Pencil (.pen) design frame
7. **create-page-from-pencil**: Implements responsive React, Astro, or HTML pages from Pencil (.pen) design files, copying images from `pencil/images/` to the assets directory
8. **convert-images-to-webp**: Converts PNG and JPEG images to WebP format for optimized file sizes
9. **generate-wireframe-catalog**: Generates comprehensive wireframe catalog documentation
10. **generate-catalog-pdf**: Converts wireframe catalog HTML to PDF, with SVG-to-PNG conversion for reliable rendering
...
1. **Check if `spec.md` exists**: The skill requires `spec.md` to discover Page and Component wireframe types. If missing, suggest `/generate-spec` first.
2. **Check if wireframes exist**: Look in `docs/wireframes/` for Page wireframe directories listed in `spec.md`
3. **Check if `pencil/design.pen` exists**: The `.pen` file must be created manually in the Pencil application before Claude can work with it. If it does not exist, ask the user to:
...
4. **If prerequisites are met**: Use `/create-pencil-design`
   - The skill automatically discovers all Page wireframes and breakpoints from `spec.md` and `docs/wireframes/`
   - Component wireframes are built as reusable Pencil components within each Page design
5. **If no wireframes**: Suggest creating them with `/create-page-wireframe` first
6. **Output**: High-fidelity design frames for all Page wireframes × all breakpoints in the active `.pen` file
...
4. /create-pencil-design
   ...
   -> Reads: pencil/coverage.md (if exists, to skip already-applied wireframes)
   -> Scans: docs/wireframes/ (to find all breakpoints for each Page wireframe)
   -> Output: Standalone reusable Pencil components for all Component wireframes, then high-fidelity design frames for all Page wireframes × all breakpoints referencing those components. Updates pencil/coverage.md after completion.
   ...
   -> Requires: Steps 0, 1, and 3 complete
...
generate-spec (0) [Optional - generates spec.md from requirements]
    └── create-page-wireframe (1)
            ...
            └── [MANUAL] Create pencil/design.pen in Pencil app (3)
                    └── create-pencil-design (4) ←→ report-pencil-coverage [auto-runs after design]
                            └── create-page-from-pencil (5)
```

Note: `report-pencil-coverage` is automatically invoked by `create-pencil-design` after completion. It can also be run standalone at any time to regenerate `pencil/coverage.md` from the current `.pen` file state.
...
4. **Verify prerequisites**
   - Before `/create-pencil-design`: Check `spec.md` exists, `docs/wireframes/` contains Page wireframe directories, and `pencil/design.pen` exists
...
**User tries**: `/create-pencil-design`
**No `spec.md` exists**
...
**User tries**: `/create-pencil-design`
**No wireframes exist in `docs/wireframes/`**

**Claude should respond**:
"No wireframes found in `docs/wireframes/`. Let's create them first:"
