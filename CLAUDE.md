...
│       ├── coverage.md                       # Coverage manifest (auto-generated by generate-svg-wireframes)
│       └── {NNNN}/                           # Wireframe ID directory
...
2. **generate-svg-wireframes**: Creates SVG wireframe designs from `spec.md` in the project root for multiple breakpoints (defaults to [1024, 768, 375]). Generates responsive wireframes for all specified viewport widths in a single invocation. Reads `docs/wireframes/coverage.md` to skip already-applied wireframes (incremental generation). Updates the coverage manifest after completion.
...
9. **generate-pages-from-wireframes**: Implements responsive React, Astro, or HTML pages directly from SVG wireframes and spec.md, bypassing the Pencil design step. Always generates all Page wireframes from spec.md. Uses project theme colors instead of wireframe placeholder grays. Faster prototyping path with placeholder images.
...
# ----------------------------------------
4b. /generate-pages-from-wireframes [framework] [output-path]
   ...
   -> Why: Faster shortcut that skips Pencil design step; less visual fidelity; always generates all Page wireframes
