...
8. **Section-specific responsive patterns**:
   ...
   **Header/Navigation (Astro):**
   ```astro
   <!-- Mobile: smaller padding, compact nav -->
   class="px-4 {prefix}:px-12 py-4 {prefix}:py-6 flex justify-between items-center"

   <!-- Logo -->
   class="text-xl {prefix}:text-2xl font-bold"

   <!-- Nav links -->
   class="flex gap-4 {prefix}:gap-8"
   class="text-sm {prefix}:text-base"
   ```
   ...
   **Hero Section (Astro):**
   ```astro
   <!-- Container: stack on mobile, side-by-side on desktop -->
   class="px-4 {prefix}:px-24 py-8 {prefix}:py-20 flex flex-col {prefix}:flex-row gap-8 {prefix}:gap-16"
